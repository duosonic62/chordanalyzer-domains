...
func TestNewTensionCode(t *testing.T) {
	actual, _ := NewTensionCode(&scale.Notes.C, Major, []scale.Interval{scale.Intervals.Major6})
	if actual.name != "C6" {
		t.Error("Expected: C6, but actual: " + actual.name)
	}

	// under 3 notes
	_, err := NewTensionCode(&scale.Notes.C, Major, []scale.Interval{})
	if err == nil {
...
func TestNewTensionCodeWithNameFrom(t *testing.T) {
	actual, _ := NewTensionCodeWithName("Cm7b5", &scale.Notes.C, Diminish, []scale.Interval{scale.Intervals.Major7})
...
func TestTension_Contains(t *testing.T) {
	c7, _ := NewTensionCode(&scale.Notes.C, Major, []scale.Interval{scale.Intervals.Major7})
